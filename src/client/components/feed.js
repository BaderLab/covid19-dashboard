        ...
        h('div.feed-title', [
          h('div.feed-status', [
            h(`div.feed-status-result.feed-status-result-${result ? 'positive' : 'negative'}`, [
              h(`i.fas.fa-fw.fa-${result ? 'virus' : 'virus-slash'}`)
            ]),
            h('div.feed-status-score', [
              h('div.feed-status-score-bar', { style: { width: `${100 * statusScore}%` } })
            ])
          ]),
          h('div.feed-patient', [
            ...
            h('.div.feed-symptoms-info', [
              ...
              h('div.feed-symptoms-score', [
                h('div.feed-symptoms-score-bar', { style: { width: `${100 * symptomsScore}%` } })
              ])
            ])
          ]),
          h('div.feed-date', [
